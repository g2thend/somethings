...
mysql -h$HOST -P$PORT -u$USER -p$PASS $DB -e "INSERT INTO log (user_id, log_trusted_ip, log_trusted_port, log_remote_ip, log_remote_port, log_start_time, log_end_time) VALUES('$common_name','$trusted_ip', '$trusted_port','$ifconfig_pool_remote_ip', '$remote_port_1', now(),NULL)"

#echo " $common_name   $trusted_ip  $trusted_port  $ifconfig_pool_remote_ip  $remote_port_1  $bytes_received  $bytes_sent "   >>/etc/openvpn/scripts/connect.log
