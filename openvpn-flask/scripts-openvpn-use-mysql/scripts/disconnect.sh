...
mysql -h$HOST -P$PORT -u$USER -p$PASS $DB -e "UPDATE log SET log_end_time=now() WHERE log_trusted_ip='$trusted_ip' AND log_trusted_port='$trusted_port' AND user_id='$common_name' AND log_end_time IS NULL"

#echo " $common_name   $trusted_ip  $trusted_port   $bytes_received  $bytes_sent "   >>/etc/openvpn/scripts/disconnect.log
